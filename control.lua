...
Event.register(Event.core_events.configuration_changed, function(event)
    if event.mod_changes[MOD.name] and event.mod_changes[MOD.name].old_version and
        event.mod_changes[MOD.name].new_version then
...
Event.register("resource-labels-add", function(event)
    ...
    if stop + cooldown <= event.tick then
        ...
        for chunk in surface.get_chunks() do
            if force.is_chunk_charted(surface, chunk) and surface.count_entities_filtered {
                area = Chunk.to_area(chunk),
                type = "resource"
            } > 0 then
...
function scheduleLabelingForChunk(scheduledTick, player, surface, chunk)
    ...
    table.insert(global.chunksToLabel[scheduledTick], {
        player = player,
        surface = surface,
        area = stdlibAreaToFactorioArea(Chunk.to_area(chunk))
    })
end
...
function labelResourcesOnPosition(player, surface, position, knownPositions)
    ...
    table.filter(resources, function(resource)
        return labelIsEnabled(table.first(resource))
    end)
    local collapsedResources = collapseMultitileResources(resources)
    table.each(collapsedResources, function(resource)
        table.each(resource, function(entity)
...
function createLabelForResourcePatch(player, surface, patch)
    ...
    if not isAlreadyLabeled(force, surface, patch) then
        ...
        if signalID and signalID.type == "item" and #patch <=
            global.settings["resource-labels-minimum-resource-entity-count"] then
            return
        end
...
function isAlreadyLabeled(force, surface, patch)
    ...
    return table.find(labelData, function(labeledResourceData)
        return labeledResourceData.surface.name == surface.name and labeledResourceData.type == type and
                   Position.inside(centerPosition, labeledResourceData.bounds)
    end)
end
...
Event.register("resource-labels-remove", function(event)
    local player = Game.get_player(event.player_index)
    local force = player.force
