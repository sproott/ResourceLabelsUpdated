...
Event.register(
  ...
  function(event)
    if
      event.mod_changes[MOD.name] and event.mod_changes[MOD.name].old_version and
        event.mod_changes[MOD.name].new_version
     then
  ...
  function(event)
    ...
    if stop + cooldown <= event.tick then
      ...
      for chunk in surface.get_chunks() do
        if
          force.is_chunk_charted(surface, chunk) and
  ...
  function(event)
    if global.chunksToLabel[event.tick] ~= nil then
      if global.settings['resource-labels-use-old-algorithm'] then
        table.each(
          global.chunksToLabel[event.tick],
          function(data)
...
function createLabelForResourcePatch(player, surface, patch)
  ...
  if not isAlreadyLabeled(force, surface, patch) then
    ...
    if
      signalID and signalID.type == 'item' and
        #patch <= global.settings['resource-labels-minimum-resource-entity-count']
     then
      return
    end
